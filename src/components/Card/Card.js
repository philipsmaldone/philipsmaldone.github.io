  ...
  return (
        <div className="max-w-md m-10 bg-white rounded-xl shadow-md overflow-hidden md:max-w-full border-sky-700 border-b-2">
            <div className="md:flex">
                ...
                <div className="p-8">
                    <div className="tracking-wide text-sm text-sky-700">{date}</div>
                    ...
                    <span className={'text-sm hover:border-b-2 cursor-pointer border-sky-700 border-b-1'} onClick={toggleExpansion}>{description ? isExpanded ? 'Show Less' : 'Show More': ''}</span>
                </div>
            </div>
        </div>
  )
